...
> **提示**：如果通过 `npx skills add zhyt1985/turf-skills` 安装，可直接使用 `npx turf-skills`，无需全局安装。
...
```bash
npm install -g turf-skills
npx turf-skills --action distance --input '{"type":"Point","coordinates":[120,30]}' --input2 '{"type":"Point","coordinates":[121,31]}'
```
...
| 你说                                                        | Claude 执行                                                                                                                                                                        |
| ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "计算北京到上海的距离"                                      | `npx turf-skills --action distance --input '{"type":"Point","coordinates":[116.397428,39.90923]}' --input2 '{"type":"Point","coordinates":[121.473701,31.230416]}' --units kilometers` |
| "天安门是否在这个多边形内？"                                | `npx turf-skills --action booleanPointInPolygon --input '...' --input2 '...'`                                                                                                          |
| "给这个点创建 5 公里缓冲区"                                 | `npx turf-skills --action buffer --input '...' --radius 5 --units kilometers`                                                                                                          |
| "对 points.geojson 中的点集计算凸包，保存到 result.geojson" | `npx turf-skills --action convex --file points.geojson --output result.geojson`                                                                                                        |
| "计算 polygon.geojson 的面积"                               | `npx turf-skills --action area --file polygon.geojson`                                                                                                                                 |
| "生成北京区域的六边形网格"                                  | `npx turf-skills --action hexGrid --bbox '[116,39,117,40]' --cellSide 10 --units kilometers`                                                                                           |
| "把这些点聚类成 3 组"                                       | `npx turf-skills --action clustersKmeans --file points.geojson --numberOfClusters 3`                                                                                                   |
| "查找离 [120.3, 30.3] 最近的点"                             | `npx turf-skills --action nearestPoint --input '...' --file2 points.geojson`                                                                                                           |
| "合并这两个多边形"                                          | `npx turf-skills --action union --input '...' --input2 '...'`                                                                                                                          |
| "这条线有多长（公里）？"                                    | `npx turf-skills --action length --file line.geojson --units kilometers`                                                                                                               |
